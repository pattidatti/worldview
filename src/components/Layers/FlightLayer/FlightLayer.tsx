...
// Simple airplane SVG as data URI, pointing up (north)
...
export function FlightLayer({ onSelect }: FlightLayerProps) {
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                if (!cancelled) setLayerLoading('flights', false);
            }
        };
    ...
    useEffect(() => {
        ...
        const handler = viewer.selectedEntityChanged.addEventListener((entity: Entity | undefined) => {
            ...
            const flight = flights.find((f) => f.icao24 === entity.id);
            if (!flight) return;
